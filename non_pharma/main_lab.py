...
if __name__ == '__main__':
    ...
    medical_supplies = Concept("Lab Diagnostics")
    ...
    for row in range(2, sheet.max_row + 1):
        count += 1
        full_name = sheet.cell(row=row, column=4).value
        concept_class = sheet.cell(row=row, column=8).value
        # Extras
        registration_code = sheet.cell(row=row, column=2).value
        storage_condition = sheet.cell(row=row, column=17).value
        device_subset = sheet.cell(row=row, column=18).value
        shelf_life = sheet.cell(row=row, column=20).value
        country_of_origin = sheet.cell(row=row, column=24).value
        test_discipline = sheet.cell(row=row, column=32).value
        mah = sheet.cell(row=row, column=21).value
        manufacturer_name = sheet.cell(row=row, column=22).value


        if full_name:
            non_pharm = Concept(full_name)
