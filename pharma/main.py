...
def add_concepts_to_list(r):
    brand_proprietary_name = sheet.cell(row=r, column=3).value
    inn = sheet.cell(row=r, column=4).value
    strength = sheet.cell(row=r, column=5).value
    dosage_form = sheet.cell(row=r, column=6).value
    route_of_administration = sheet.cell(row=r, column=7).value
    pack_size = sheet.cell(row=r, column=8).value
    shelf_life = sheet.cell(row=r, column=12).value
    storage_conditions = sheet.cell(row=r, column=13).value
    registration_number = sheet.cell(row=r, column=15).value
    country_of_origin = sheet.cell(row=r, column=21).value
    atc_code = sheet.cell(row=r, column=22).value
    product_visual_descriptions = sheet.cell(row=r, column=29).value
    therapeutic_classification_level_3 = sheet.cell(row=r, column=23).value
    # More additions
    ectd_product_id = sheet.cell(row=r, column=2).value
    ...
    if brand_proprietary_name:
        ...
        pharm.extras = {"atc_code": atc_code, "strength": strength, "pack_size": pack_size,
                        ...
                        "ectd_product_id": ectd_product_id}
        ...
        pharm.pk = "{}-{}-{}-{}-{}-{}-{}-{}".format(
            brand_proprietary_name, inn, strength, dosage_form,
    ...
    for concept_1 in concepts_list:
        for concept_2 in concepts_list:
            if (concept_1 != concept_2 and concept_1.pk == concept_2.pk
                ...
                concept_2_pack_size = concept_2.extras['pack_size']

                if type(concept_1_pack_size) is not list:
                    concept_1_pack_size = [concept_1_pack_size]
                if type(concept_2_pack_size) is not list:
                    concept_2_pack_size = [concept_2_pack_size]

                concept_1.extras['pack_size'] = concept_1_pack_size + concept_2_pack_size
...
def upload_sub_domains():
    for tc in sub_domains_list:
        sub_domain = Concept(brand_proprietary_name=None, inn=None,
                             product_visual_descriptions=None)
        sub_domain.concept_class = "Subdomain"
        sub_domain.datatype = "N/A"
        sub_domain.names = [ConceptName(name=tc, ctype="FULLY SPECIFIED")]
        sub_domain.descriptions = [ConceptDescription(tc)]
        sub_domain.type = "Concept"
        resp_dict = post_concept(concept=sub_domain, parent_id=None,
...
if __name__ == '__main__':
    ...
    for concept in concepts_list:
        ...
        try:
            ...
            post_concept(concept=concept, parent_id=tcc_details['id'],
                         parent_concept_name=tcc_details['name'],
